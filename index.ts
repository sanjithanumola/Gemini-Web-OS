    ...
    async function navigateToUrl(url: string): Promise<void> {
        ...
        try {
            ...
            try {
                    ...
                    if (!await initializeGeminiIfNeeded('initAiBrowser')) {
                        iframe.src = 'data:text/html;charset=utf-8,' + encodeURIComponent(`
                            ...
                            <head>
                                <title>${domain}</title>
                                <style>
                                    body { font-family: 'Comic Sans MS', cursive, sans-serif; background: #000080; color: #00ff00; padding: 20px; text-align: center; }
                                    h1 { color: #ff00ff; text-shadow: 2px 2px #ffff00; }
                                    .marquee { background: #ff0000; color: #ffffff; padding: 5px; font-weight: bold; }
                                    .card { background: #000; border: 3px ridge #c0c0c0; padding: 15px; margin: 20px auto; max-width: 80%; }
                                </style>
                            </head>
                            <body>
                                <div class="marquee"><marquee>Welcome to ${domain}! Best viewed in 800x600 resolution!</marquee></div>
                                <h1>🌐 ${domain} 🌐</h1>
                                <div class="card">
                                    <p>Welcome to the official 90s web portal for <strong>${domain}</strong>!</p>
                                    <p>🚧 Under Construction 🚧</p>
                                    <p>Visitor Counter: 009,421</p>
                                </div>
                            </body>
                            </html>
                        `);
                ...
                const websitePrompt = `
                Create a complete HTML string for a nostalgic 90s-style website for "${domain}".
                Include garish 90s styling (neon text, table layouts, comic sans font), content specific to "${domain}", scrolling marquee, retro emojis, and a visitor counter. Return ONLY valid raw HTML.
                `;
                 // @ts-ignore
                ...
                let htmlContent = result?.candidates?.[0]?.content?.parts?.[0]?.text?.replace(/```html|```/g, '').trim() || "";
                
                if (!htmlContent.includes("<html")) {
                    htmlContent = `<!DOCTYPE html><html><head><title>${domain}</title><style>body{font-family:"Comic Sans MS";background:#008080;color:yellow;}marquee{background:purple;color:white;padding:4px;}</style></head><body><marquee>Welcome to ${domain}!</marquee><h1>${domain}</h1><div>${htmlContent}</div></body></html>`;
                }
                iframe.src = 'data:text/html;charset=utf-8,' + encodeURIComponent(htmlContent);
                addressBar.value = url;
            } catch (e: any) {
                console.warn("AI website generation fallback triggered:", e);
                iframe.src = 'data:text/html;charset=utf-8,' + encodeURIComponent(`
                    <!DOCTYPE html>
                    <html>
                    <head><title>${domain}</title><style>body{font-family:'Courier New',monospace;background:#1e1b4b;color:#f8fafc;padding:20px;text-align:center;}</style></head>
                    <body>
                        <h2>🌐 ${domain}</h2>
                        <p>Welcome to ${domain}! Gemini AI Portal active.</p>
                    </body>
                    </html>
                `);
            } finally {
    ...
    if (maximizeButton) {
        maximizeButton.addEventListener('click', (e) => {
            ...
            if (windowElement.dataset.maximized === 'true') {
                windowElement.style.top = windowElement.dataset.prevTop || '60px';
                windowElement.style.left = windowElement.dataset.prevLeft || '100px';
                windowElement.style.width = windowElement.dataset.prevWidth || '520px';
                windowElement.style.height = windowElement.dataset.prevHeight || '420px';
                ...
                windowElement.style.top = '38px';
                windowElement.style.left = '0px';
                windowElement.style.width = '100vw';
                windowElement.style.height = 'calc(100vh - 98px)';
...
async function critiquePaintDrawing(): Promise<void> {
    ...
    try {
        ...
        const imagePart = { inlineData: { data: base64Data, mimeType: "image/jpeg" } };
         // @ts-ignore
        const result = await geminiInstance.models.generateContent({ model: "gemini-2.5-flash", contents: [{ role: "user", parts: [ { text: prompt }, imagePart] }] });
...
async function initializeGeminiIfNeeded(context: string): Promise<boolean> {
    ...
    try {
        ...
        if (!apiKey) {
            console.warn(`[Gemini Init] API Key missing in context ${context}`);
            return false;
        }
         // @ts-ignore
        ...
        console.warn(`Failed Gemini initialization in ${context}:`, error);
        return false;
    }
}

// --- Modern Top Bar, Themes & Control Center Setup ---
...
const closeControlCenterBtn = document.getElementById('close-control-center');
const controlCenterPanel = document.getElementById('control-center');

if (controlCenterBtn && controlCenterPanel) {
    controlCenterBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        controlCenterPanel.classList.toggle('active');
    });
}
if (closeControlCenterBtn && controlCenterPanel) {
...
document.addEventListener('click', (e) => {
    if (controlCenterPanel?.classList.contains('active') &&
        !controlCenterPanel.contains(e.target as Node) &&
        !controlCenterBtn?.contains(e.target as Node)) {
        controlCenterPanel.classList.remove('active');
    }
});
...
themeChips.forEach(chip => {
    chip.addEventListener('click', () => {
        const themeId = (chip as HTMLElement).dataset.themeId;
        if (themeId && desktop) {
            desktop.setAttribute('data-theme', themeId);
...
const launcherSearch = document.getElementById('launcher-search') as HTMLInputElement | null;
if (launcherSearch) {
    launcherSearch.addEventListener('input', () => {
        const query = launcherSearch.value.toLowerCase().trim();
        const startItems = document.querySelectorAll('.start-menu-item');
